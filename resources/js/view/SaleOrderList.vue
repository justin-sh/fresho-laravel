    ...
    <BCard>
        ...
        <template #footer>
            <BPagination v-model="currentPage" :total-rows="data.length" :per-page="page_size" limit="7"
                         @update:model-value="goTableHead" aria-controls="so-list"></BPagination>
        </template>

        <BTable id="so-list" striped hover :busy="loading"
                :sort-by="[{key: 'pickupAt', order: 'desc'},{key: 'title', order: 'asc'}]"
...
const fields = [
    ...
    {key: 'pickupAt', label: 'Pickup At', sortable: true},
...
const load_data = async function () {
    loading.value = true
    data.value = (await getSoList()).data.data
    loading.value = false
}
...
const gotoDetail = (item: TableItem, index: number, event: MouseEvent) => {
    router.push({"name": 'saleOrder', "params": {"id": item.id}})
}
onMounted(load_data)
</script>
